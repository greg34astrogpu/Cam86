...
type
 TMyImage = class(TObject)
  ...
  procedure FitFileCol(filename:string);
  procedure Calk;
  procedure CalkGis;
  procedure CalkGisCol;
  procedure Dark0;
  procedure DarkAdd;
  procedure DarkSub;
...
begin
 fillchar(gism,sizeof(gism),0);
 migis:=0;magis:=65535;md:= 0.001*Height*Width;
 kol:=0;kol2:=0;
 for y:=1 to Height-2 do
    for x:=1 to Width-3 do
      begin
       inc(gism[bufi2[3*x+1+3*y*Width]]);
      end;
     ...
     for x:=0 to 65535 do
      begin
       kol:=kol+gism[x];
       kol2:=kol2+gism[65535-x];
       if kol < md then migis:=x;
       if kol2 < md then magis:=65535-x;
      end;
      if migis = magis then magis:=migis+1;
      digis:=magis-migis;
end;
...
procedure TMyImage.FitFileCol(filename:string);
const
fitst0 ='SIMPLE  =                    T                                                  ';
fitst1 ='BITPIX  =                   16 /8 unsigned int, 16 & 32 int, -32 & -64 real     ';
fitst2 ='NAXIS   =                    3 /number of axes                                  ';
fitst3 ='NAXIS1  =                 1000 /fastest changing axis                           ';
fitst4 ='NAXIS2  =                 1000 /next to fastest changing axis                   ';
fitst5 ='NAXIS3  =                    3                                                  ';
fitst6 ='BSCALE  =   1.0000000000000000 /physical = BZERO + BSCALE*array_value           ';
fitst7 ='BZERO   =   32768.000000000000 /physical = BZERO + BSCALE*array_value           ';
fitst8 ='INPUTFMT= ''Raw Binary'' /        Format of file from which image was read      ';
fitst9 ='CBLACK  =                    0 /Initial display black level in ADUs             ';
fitst10='CWHITE  =                 1024 /Initial display white level in ADUs             ';
fitst11='EXPOSURE=                  0.0 /exposure time in seconds                        ';
fitst12='CCD-TEMP=                 0.00 /CCD temperature in C                            ';
fitst13='END';
var
x,y:integer;
f2:file;
probel:array [0..1916-80] of byte;
st:string;
st3:array[0..79] of char;
img:array[0..9999] of smallint;
begin
 ...
 st3[30]:=st[5];
 blockwrite(f2,st3,80);
 blockwrite(f2,fitst13,3);
 blockwrite(f2,probel,sizeof(probel));
  for y:=0 to Height-1 do
   begin
    for x:= 0 to Width-1 do img[x]:= $80+swap(bufi2[3*x+0+y*3*Width]);
    blockwrite(f2,img,2*Width);
   end;
  for y:=0 to Height-1 do
   begin
    for x:= 0 to Width-1 do img[x]:= $80+swap(bufi2[3*x+1+y*3*Width]);
    blockwrite(f2,img,2*Width);
   end;
  for y:=0 to Height-1 do
   begin
    for x:= 0 to Width-1 do img[x]:= $80+swap(bufi2[3*x+2+y*3*Width]);
    blockwrite(f2,img,2*Width);
   end;
 closefile(f2);
end;
